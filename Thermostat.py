...
#------------------------------------------------------------------
#    2           Developed full functionality including
#                state machine, button handling, display
#                management, temperature sensing, and
...
class TemperatureMachine(StateMachine):
    ...
    def updateLights(self):
        ...
        if self.current_state.id == 'off':
            ...
            if temp < self.setPoint:
                # Temperature is below setpoint - blink red light
                redLight.pulse(fade_in_time=1, fade_out_time=1, n=None, background=True)
            else:
                # Temperature is at or above setpoint - solid red light
                redLight.value = 1.0

        elif self.current_state.id == 'cool':
            # Cooling Mode
            if temp > self.setPoint:
                # Temperature is above setpoint - blink blue light
                blueLight.pulse(fade_in_time=1, fade_out_time=1, n=None, background=True)
            else:
                # Temperature is at or below setpoint - solid blue light
                blueLight.value = 1.0
    ...
    def setupSerialOutput(self):
        ...
        output = f"{self.current_state.id}, T = {floor(self.getFahrenheit())}, P = {self.setPoint}"
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            if(altCounter < 6):
                ...
                lcd_line_2 = f"Mode:{self.current_state.id} Set:{self.setPoint}F"
...
##
blueButton = Button(24) #blue Button
blueButton.when_pressed = tsm.processTempIncButton

##
## Configure our White button to use GPIO 25 and to execute
## the method to cycle the thermostat when pressed.
##
whiteButton = Button(25) #white Button 
whiteButton.when_pressed = tsm.processTempStateButton

##
## Configure our Red button to use GPIO 12 and to execute
## the function to decrease the setpoint by a degree.
##
redButton = Button(12) #red Button
